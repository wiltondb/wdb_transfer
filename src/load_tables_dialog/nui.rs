...
pub(super) struct LoadTablesDialogNui {
    inner: Rc<RefCell<LoadTablesDialog>>,
    inner_events: Rc<LoadTablesDialogEvents>,
    default_handler: RefCell<Option<nwg::EventHandler>>
}

impl LoadTablesDialogNui {
    pub(super) fn result(&mut self) -> LoadTablesDialogResult {
        self.inner.borrow_mut().result()
    }
}

impl nwg::NativeUi<LoadTablesDialogNui> for LoadTablesDialog {
    fn build_ui(mut dialog: LoadTablesDialog) -> Result<LoadTablesDialogNui, nwg::NwgError> {
        let mut events: LoadTablesDialogEvents = Default::default();
        dialog.c.build()?;
        ...
        let wrapper = LoadTablesDialogNui {
...
impl Drop for LoadTablesDialogNui {
    fn drop(&mut self) {
